...
class ILLEGALAmmo {
    ...
    postDBLoad(container) {
        ...
        const CalibreGroup = [/*0*/ "Small Calibre", /*1*/ "Medium Calibre", /*2*/ "Large Calibre", /*3*/ "Special Calibre"];
        ...
        if (debug)
            console.log(db.templates.handbook.Items.find(e => e.Id == "7.62x25mm TT ILLEGAL"));
        if (debug)
            console.log("Locale");
        if (debug)
            console.log(db.locales.global.en.templates["7.62x25mm TT ILLEGAL"]);
        //Define Ammo End
    ...
    addToHandbook(HandbookID, AmmoID, Calibre) {
    ...
    addToContainerBlacklist(ContainerBlacklistID, AmmoID, Calibre) {
        const logger = tsyringe_1.container.resolve("WinstonLogger");
        const db = tsyringe_1.container.resolve("DatabaseServer").getTables();
        ...
        for (let id in db.templates.items) {
            if (AdvAmmoConfig["Removed From Secured Container"] == "false") {
                //Do nothing
            }
            else {
                if (AdvAmmoConfig["Removed From Secured Container"] == "true") {
                    ...
                    if (SimAmmoConfig["Remove ALL From Secured Container"] == "false") {
                        //Do nothing
                    }
                    else {
                        if (SimAmmoConfig["Remove ALL From Secured Container"] == "true") {
                            if (db.templates.items[id]._parent == ContainerBlacklistID) {
                                db.templates.items[id]._props.Grids[0]._props.filters[0].ExcludedFilter.push(AmmoID);
                            }
                        }
    ...
    CreateLootTables(AmmoID, Chance) {
        const SimAmmoConfig = this.modConfig["Simple Ammunition Config"];
    ...
    cloneAmmo(AmmoToClone, AmmoID, Calibre) {
        const db = tsyringe_1.container.resolve("DatabaseServer").getTables();
        const logger = tsyringe_1.container.resolve("WinstonLogger");
    ...
    AddToAssortTable(Itrade, AmmoID, Calibre, DefaultLoyaltyLevel) {
        const logger = tsyringe_1.container.resolve("WinstonLogger");
        const ROUBLE_ID = "5449016a4bdc2d6f028b456f";
        const AdvAmmoConfig = this.modConfig["Advanced Ammunition Config"][Calibre][AmmoID];
        const SimAmmoConfig = this.modConfig["Simple Ammunition Config"];
        const new762x25mmTTILLEGAL = {
            _id: AmmoID,
            _tpl: AmmoID,
            parentId: "hideout",
            slotId: "hideout",
        ...
        if ([1, 2, 3, 4].includes(AdvAmmoConfig["Loyalty Level Requirement"])) {
            ...
            if ([1, 2, 3, 4].includes(SimAmmoConfig["Loyalty Level Requirement"])) {
                ...
                logger.logWithColor("No manual configuration of \"Loyalty Level Requirement\" found for " + String(AmmoID) + " using default settings.", LogTextColor_1.LogTextColor.blue);
