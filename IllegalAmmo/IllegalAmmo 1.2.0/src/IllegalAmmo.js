...
class ILLEGALAmmo {
    constructor() {
        ...
        this.debug = this.castBool(this.modConfig["General Config"]["Debug Log"]);
        this.log = this.castBool(this.modConfig["General Config"]["Detailed Server Log"]);
        this.logger = tsyringe_1.container.resolve("WinstonLogger");
        this.db = tsyringe_1.container.resolve("DatabaseServer").getTables();
        this.mod = "skybellrock-IllegalAmmo";
    }
    preAkiLoad(container) {
        this.registerProfileImage(container);
        this.setupTraderUpdateTime(container);
        this.logger.logWithColor("Loading: Illegal AMMO", LogTextColor_1.LogTextColor.BLUE);
    }
    postDBLoad(container) {
        this.db = container.resolve("DatabaseServer").getTables();
        const CaliberGroup = [/*0*/ "Small Calibre", /*1*/ "Medium Calibre", /*2*/ "Large Calibre", /*3*/ "Special Calibre"];
        const LoyaltyGroup = ["1", "2", "3", "4"];
        ...
        var CaliberID = [];
        var CaliberThisIs = [];
        var LoyaltyGroupThisIs = [];
        var Descript = [];
        var ValidID = "";
        var _V_ID_Index = [];
        var _V_ToAddList = [];
        var _V_ToValueList = [];
        var _V_AmmoProbabilityEachContainer = [];
        //Define Ammo Start
        this.showCaliber();
        let CaliberClass;
        (function (CaliberClass) {
            CaliberClass[CaliberClass["Small"] = 0] = "Small";
            CaliberClass[CaliberClass["Medium"] = 1] = "Medium";
            CaliberClass[CaliberClass["Large"] = 2] = "Large";
            CaliberClass[CaliberClass["Special"] = 3] = "Special";
        })(CaliberClass || (CaliberClass = {}));
        let LoyaltyLevel;
        (function (LoyaltyLevel) {
            LoyaltyLevel[LoyaltyLevel["One"] = 0] = "One";
            LoyaltyLevel[LoyaltyLevel["Two"] = 1] = "Two";
            LoyaltyLevel[LoyaltyLevel["Three"] = 2] = "Three";
            LoyaltyLevel[LoyaltyLevel["Four"] = 3] = "Four";
        })(LoyaltyLevel || (LoyaltyLevel = {}));
        function appendBullet(_Name, _AmmoID, _CloneID, _CaliberID, _CaliberThisIs, _LoyaltyGroupThisIs, _Description) {
            Name.push(_Name);
            AmmoID.push(_AmmoID);
        ...
        appendBullet("9x19mm ILLEGAL", "9x19mm ILLEGAL", "5efb0da7a29a85116f6ea05f", "Caliber9x19PARA", CaliberClass.Small, LoyaltyLevel.Two, "Pew pew.");
        appendCylinder("624c3074dbbd335e8e6becf3");
        ...
        appendBullet(".357 Magnum ILLEGAL", ".357 Magnum ILLEGAL", "62330b3ed4dc74626d570b95", "", CaliberClass.Medium, LoyaltyLevel.Two, "Real cowboys need real ammo!");
        appendCylinder("619f54a1d25cbd424731fb99");
        appendCylinder("61a4cda622af7f4f6a3ce617");
        ...
        appendBullet("12/70mm ILLEGAL buckshot", "12/70mm ILLEGAL buckshot", "5c0d591486f7744c505b416f", "Caliber12g", CaliberClass.Special, LoyaltyLevel.Two, "Hopefully these pellets are better than the ones you give your pet to eat. Pets have tastebuds too you know.");
        appendCylinder("60dc519adf4c47305f6d410d");
        appendAmmoToAllChamber("5580223e4bdc2d1c128b457f");
        appendBullet("12/70mm ILLEGAL slug", "12/70mm ILLEGAL slug", "5d6e68e6a4b9361c140bcfe0", "Caliber12g", CaliberClass.Special, LoyaltyLevel.Three, "Turns your shotgun into a sniper rifle, so you can finally shoot those pesky birds the way you are meant to.");
        appendCylinder("60dc519adf4c47305f6d410d");
        appendAmmoToAllChamber("5580223e4bdc2d1c128b457f");
        appendBullet("12/70mm ILLEGAL BOOM", "12/70mm ILLEGAL BOOM", "5d6e68a8a4b9360b6c0d54e2", "Caliber12g", CaliberClass.Special, LoyaltyLevel.Four, "Says BOOM. Don't kill yourself.");
        appendCylinder("60dc519adf4c47305f6d410d");
        appendAmmoToAllChamber("5580223e4bdc2d1c128b457f");
        ...
        appendBullet("40x46mm ILLEGAL (HE) grenade", "40x46mm ILLEGAL (HE) grenade", "5ede474b0c226a66f5402622", "Caliber40x46", CaliberClass.Special, LoyaltyLevel.Four, "Turns enemies into mush.");
        appendCylinder("627bce33f21bc425b06ab967");
        //Add Bullet Data End
        this.logger.logWithColor("Loading Through: " + String(AmmoID.length) + " Bullets", LogTextColor_1.LogTextColor.GREEN);
        ...
        for (let i = 0; i < AmmoID.length; i++) {
            ...
            if (this.log)
                this.logger.logWithColor("Loading: " + String(i + 1) + "/" + String(AmmoID.length) + " Bullets", LogTextColor_1.LogTextColor.GREEN);
        }
        if (this.log)
            this.logger.logWithColor("Applying Additions: " + String(_V_ID_Index.length) + " Additions", LogTextColor_1.LogTextColor.GREEN);
        for (let i = 0; i < _V_ID_Index.length; i++) {
            ...
            if (this.log)
                this.logger.logWithColor("Loading: " + String(i + 1) + "/" + String(_V_ID_Index.length) + " Additions", LogTextColor_1.LogTextColor.GREEN);
        }
        //Define Ammo End
        ...
        if (this.debug)
            console.log(this.db.templates.handbook.Items.find(e => e.Id == "7.62x25mm TT ILLEGAL"));
        if (this.debug)
            console.log("Locale");
        if (this.debug)
            console.log(this.db.locales.global.en.templates["7.62x25mm TT ILLEGAL"]);
        //Define Ammo End
        ...
        const traderLocales = Object.values(this.db.locales.global);
        ...
        //this.showWeaponMod()
        container.resolve("ConfigServer").getConfig(ConfigTypes_1.ConfigTypes.RAGFAIR).traders[DarkWebAmmoTrader._id] = true;
    }
    addToHandbook(HandbookID, AmmoID, Caliber) {
                ...
                for (const slotIndex in this.db.templates.items[item]._props.Slots) {
                    if (this.db.templates.items[item]._props.Slots[slotIndex]._name == "mod_magazine") {
                        for (const mags in this.db.templates.items[item]._props.Slots[slotIndex]._props.filters[0].Filter) {
                            if (this.debug)
                                console.log("Magazine: " + this.db.templates.items[item]._props.Slots[slotIndex]._props.filters[0].Filter[mags]);
    ...
    addToWeaponChamber(weapon, ammoToAdd) {
        ...
        if (weaponID._props.Chambers?.[0]?._props != undefined) {
            for (const index in weaponID._props.Chambers) {
                if (this.debug)
                    this.logger.log("Adding " + ammoToAdd + " to " + this.db.templates.items[weapon]._name, "cyan");
    ...
    addToMagazine(magazine, ammoToAdd) {
        ...
        if (this.db.templates.items[magazine] != undefined) {
            ...
            if (magazineID._props.Cartridges?.[0]?._props != undefined) {
                if (this.debug)
                    this.logger.log("Adding " + ammoToAdd + " to " + this.db.templates.items[magazine]._name, "cyan");
                ...
                if (this.log)
                    this.logger.logWithColor(magazineID._name, LogTextColor_1.LogTextColor.GREEN);
                if (this.log)
                    this.logger.logWithColor(ammoToAdd, LogTextColor_1.LogTextColor.YELLOW);
        ...
        if (weaponOrMagID._props.Slots != undefined && weaponOrMagID._id != "60db29ce99594040e04c4a27" && weaponOrMagID._id != "624c2e8614da335f1e034d8c" && weaponOrMagID._id != "61a4c8884f95bc3b2c5dc96f") {
            if (weaponOrMagID._props.Slots?.[0]?._props != undefined) {
                for (const index in weaponOrMagID._props.Slots) {
                    if (this.debug)
                        this.logger.log("Adding " + ammoToAdd + " to " + this.db.templates.items.weaponOrMagID._name, "cyan");
    ...
    showWeaponMod() {
        ...
        if (this.log)
            this.logger.logWithColor("Weapon Slot Names", LogTextColor_1.LogTextColor.CYAN);
        if (this.log)
            this.logger.logWithColor(newArray, LogTextColor_1.LogTextColor.RED);
    }
    showCaliber() {
        const ILLEGALArray = [];
        for (const item in this.db.templates.items) {
        ...
        if (this.debug)
            this.logger.logWithColor("Ammo Calibers", LogTextColor_1.LogTextColor.GREEN);
        if (this.debug)
            this.logger.logWithColor(ILLEGALArray, LogTextColor_1.LogTextColor.RED);
    }
    //Remove Ammo From Secured Containers Start
    addToContainerBlacklist(ContainerBlacklistID, AmmoID, Caliber) {
        const AdvAmmoConfig = this.modConfig["Advanced Ammunition Config"][Caliber][AmmoID];
        const SimAmmoConfig = this.modConfig["Simple Ammunition Config"];
    ...
    calculateLootChances(AmmoID, Caliber, PriceSumIn, In_ProbabilityToUse) {
        var Containers = ["5d07b91b86f7745a077a9432", "578f8782245977354405a1e3", "5c052cea86f7746b2101e8d8", "5914944186f774189e5e76c2", "5909e4b686f7747f5b744fa4", "5909d45286f77465a8136dc6", "5d6d2bb386f774785b07a77a", "5d6d2b5486f774785c2ba8ea", "578f87ad245977356274f2cc", "5909d7cf86f77470ee57d75a", "5909d5ef86f77467974efbd8", "5909d89086f77472591234a0", "5909d76c86f77471e53d2adf", "578f87a3245977356274f2cb", "578f8778245977358849a9b5"];
        let Probabilities = [];
        for (let i = 0; i < Containers.length; i++) {
    ...
    createLootTables(AmmoID, In_Table) {
    ...
    cloneAmmo(AmmoToClone, AmmoID, Caliber) {
        ...
        if (Colors.includes(AdvAmmoConfig["Icon Color"])) {
            ...
            if (Colors.includes(SimAmmoConfig["Icon Color"])) {
                ...
                this.logger.logWithColor("ERROR: NO BACKGROUND COLOR OPTION FOUND FOR " + String(AmmoID) + " IN config.json ENTRIES: \"Simple Ammunition Config\" → \"Icon Color\" OR \"Advanced Ammunition Config\" → \"" + String(Caliber) + "\" → \"" + String(AmmoID) + "\" → \"Icon Color\". DEFAULT SETTINGS AUTOMATICALLY RESTORED. PLEASE SET THE FORMER TO ANY OF THESE COLORS \"blue\", \"yellow\", \"green\", \"red\", \"black\", \"grey\", \"violet\", \"orange\", \"tracerYellow\", \"tracerGreen\" OR \"tracerRed\" PARANTHESIS INCLUDED, AND THE LATTER TO ANY DESIRED COLOR FOR THAT SPECIFIC AMMO, OR \"\" TO USE THE COLOR FROM \"Simple Ammunition Config\".", LogTextColor_1.LogTextColor.RED);
        ...
        AmmoPath._props.RagFairCommissionModifier = this.numberCast(GenConfig["Flea Market Fee Multiplier"], 1) + 6;
        AmmoPath._props.TracerDistance = 0.1 * (this.numberCast(SimAmmoConfig["Tracer Length Multiplier"], 1));
        if (this.log)
            this.logger.logWithColor("Ammo " + AmmoPath._id + " Added", LogTextColor_1.LogTextColor.GREEN);
    }
    ...
    cloneItem(itemtoClone, newitemID) {
        ...
        if (this.debug)
            this.logger.logWithColor(this.db.templates.items[itemtoClone]._name + " cloned", LogTextColor_1.LogTextColor.GREEN);
    }
    ...
    setupTraderUpdateTime(container) {
        ...
        const traderRefreshConfig = { traderId: DarkWebAmmoTrader._id, seconds: GenConfig["Trader Refresh In Seconds"] };
        traderConfig.updateTime.push(traderRefreshConfig);
    }
    addToAssortTable(Itrade, AmmoID, Caliber, DefaultLoyaltyLevel) {
        ...
        if ([1, 2, 3, 4].includes(AdvAmmoConfig["Loyalty Level Requirement"])) {
            ...
            if ([1, 2, 3, 4].includes(SimAmmoConfig["Loyalty Level Requirement"])) {
                ...
                if (this.log)
                    this.logger.logWithColor("No custom configuration of \"Loyalty Level Requirement\" found for " + String(AmmoID) + " using hidden default settings.", LogTextColor_1.LogTextColor.CYAN);
    ...
    castBool(InString) {
        ...
        else if (StringIn == "false") {
            OutBool = false;
        }
        return OutBool;
    }
    numberCast(InThing, Default) {
        if (typeof InThing === 'number') {
            ...
            //Very angery
            if (this.log)
                this.logger.logWithColor("Config Value Incorrect type : " + String(InThing), LogTextColor_1.LogTextColor.RED);
        }
        let _Tcast = Number(InThing);
        let _Default = Number(Default);
        if (_Default == null || isNaN(_Default)) {
            if (this.log)
                this.logger.logWithColor("Coding error", LogTextColor_1.LogTextColor.RED);
            _Default = 0;
        }
        if (isNaN(_Tcast) || _Tcast == null) {
            _Tcast = _Default;
            if (this.log)
                this.logger.logWithColor("Valid value not set for _default" + InThing, LogTextColor_1.LogTextColor.RED);
        }
